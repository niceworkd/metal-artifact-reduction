...
def get_refinement_block(model='automap_scae', in_channel=40, out_channel=1):
    if model == 'automap_scae':
        return nn.Sequential(nn.Conv2d(in_channel, 64, 3, 1, 1), nn.ReLU(True),
                             nn.Conv2d(64, 1, 3, 1, 1), nn.ReLU(True))
        ...
        return nn.Sequential(nn.Conv2d(in_channel,64,5,1,1),nn.ReLU(True),
                              nn.Conv2d(64,64,5,1,1),nn.ReLU(True),
...
class Radon(nn.Module):
    def __init__(self, in_size=None, theta=None, circle=True, dtype=torch.float):
        ...
        #self.sparse_convolutional_autoencoder = get_refinement_block('simple', 1,1)
...
class IRadon(nn.Module):
    ...
    def forward(self, x):
        ...
        reco = self.sparse_convolutional_autoencoder(reco)
        return reco
