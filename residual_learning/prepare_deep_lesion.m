...
for phase=splits
    ...
    for ii=1:numel(image_indices)/20
        ...
        fprintf('[%s][%d/%d] Processing %s\n', phase{1}, ii, numel(image_indices)/20, image_name)
        ...
        ct_file = fullfile(output_dir,'Sgt.mat');
        image = fanbeam(gt_CT, CTpara.SOD, ...
            'FanSensorGeometry', 'arc', ...
            'FanSensorSpacing', CTpara.angSize, ...
            'FanRotationIncrement', 360/CTpara.angNum);
        image = image';
        ...
        for jj=1:size(ma_CT_all, 3)
            ct_file = fullfile(output_dir, ['Pma',num2str(jj), '.mat']);
            image = ma_CT_all(:, :, jj);
            save(ct_file, 'image');
        end
        
        for jj=1:size(ma_sinogram_all, 3)
            ct_file = fullfile(output_dir, ['Sma',num2str(jj), '.mat']);
            image = ma_sinogram_all(:, :, jj);
            save(ct_file, 'image');
        end
        
        for jj=1:size(metal_trace_all, 3)
            ct_file = fullfile(output_dir, ['trace',num2str(jj), '.mat']);
            image = metal_trace_all(:, :, jj);
            save(ct_file, 'image');
        end
        
        for jj=1:size(LI_sinogram_all, 3)
            ct_file = fullfile(output_dir,['SLi',num2str(jj), '.mat']);
            image = LI_sinogram_all(:, :, jj);
            save(ct_file, 'image');
        end
        
        
         for jj=1:size(LI_CT_all, 3)
            ct_file = fullfile(output_dir, ['PLi',num2str(jj), '.mat']);
            image = LI_CT_all(:, :, jj);
            save(ct_file, 'image');
         end
        
         for jj=1:size(mask_all, 3)
            ct_file = fullfile(output_dir, ['mask',num2str(jj), '.mat']);
            image = mask_all(:, :, jj);
            save(ct_file, 'image');
         end
