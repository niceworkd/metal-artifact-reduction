...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train an PriorNet")
    ...
    def get_image(data):
        ...
        if dataset_type == "deep_lesion":
             ...
             elif dataset_opts[dataset_type]['load_all']: return data['gt'],data['prior'],data['li'],data['trace']
    ...
    criterion = nn.L1Loss();
    optimzer = optim.Adam(model.parameters(),lr=0.0001,betas=(0.5,0.999))
    for epoch in range(start_epoch, opts['num_epochs']):
        ...
        for data in (train_loader):
            ...
            # prior 2.0
            res = prior;
            sino_trace = torch.cat((prior,trace),dim = 1)
            sino_trace = torch.cat((li,sino_trace),dim=1)
            sino_trace = sino_trace.to(device)
            #print("origin_loss:%lf\n" % criterion(prior,gt))
            predict = model.forward(sino_trace)
            predict = trace*predict + (1-trace)*prior
            loss = criterion(predict,gt)
            
            sum_loss += loss
            if num % 100 == 0:
                print("prior_loss:%lf\n" % criterion(prior,gt))
                print("predict_loss:%lf\n" % loss.data.item())
            loss.backward()
            optimzer.step()
        print("avg_loss :%lf\n" % (sum_loss/num))
